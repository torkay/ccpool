...
`ccpool.rb` is the canonical Homebrew formula. It lives here for reference and review; the live tap is at [`github.com/torkay/homebrew-ccpool`](https://github.com/torkay/homebrew-ccpool).
...
3. CI runs `brew test ccpool` on the formula PR.
...
brew bump-formula-pr \
  --version=1.0.1 \
  --url=https://files.pythonhosted.org/packages/source/c/ccpool/ccpool-1.0.1.tar.gz \
  ccpool
```
...
```bash
brew install --build-from-source ./install/homebrew/ccpool.rb
```
