#!/usr/bin/env bash
# Run the ccpool Linux test suite inside a fresh Ubuntu container.
#
# Usage:
#   ./install/linux/run-tests-in-docker.sh                  # ubuntu 24.04
#   ./install/linux/run-tests-in-docker.sh 22.04            # pin version
#   CCPOOL_TEST_LINUX_KEYRING=1 ./run-tests-in-docker.sh  # opt-in keyring lane
#
...
TAG="ccpool-test:${UBUNTU_VERSION}"
...
docker run --rm \
    -e "CCPOOL_TEST_LINUX_KEYRING=${CCPOOL_TEST_LINUX_KEYRING:-0}" \
    -v "${REPO_ROOT}:/repo" \
    -w /repo \
    "$TAG"
