...
Linux's libsecret (gnome-keyring) backend can only be exercised when:
    ...
    4. The opt-in env var CMAXCTL_TEST_LINUX_KEYRING=1 is set
...
KEYRING_OPT_IN = os.environ.get("CMAXCTL_TEST_LINUX_KEYRING") == "1"
...
pytestmark = pytest.mark.skipif(
    ...
    reason="linux + secret-tool + CMAXCTL_TEST_LINUX_KEYRING=1 required",
)


def _flush_envvars(monkeypatch):
    """Don't force env-only mode for THIS test — we WANT the native backend."""
    monkeypatch.delenv("CMAXCTL_FORCE_ENV_STORAGE", raising=False)
