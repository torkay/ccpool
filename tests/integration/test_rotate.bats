#!/usr/bin/env bats
# `ccpool rotate` token replacement.
#
# `ccpool rotate` is interactive (prompts for paste). We test the surface that
...
@test "record-token writes age tracking under tokens.json" {
  ...
  run python3 -m ccpool.cli record-token alpha tester+alpha@example.com
  [ "${status}" -eq 0 ]
  [ -f "${XDG_DATA_HOME}/ccpool/tokens.json" ]
  python3 -c "
import json
d = json.loads(open('${XDG_DATA_HOME}/ccpool/tokens.json').read())
