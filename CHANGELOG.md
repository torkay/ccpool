...
### Fixed
- Mock caam binary now creates `.credentials.json` under the production-correct `<profile>/xdg_config/claude-code/` path (was at the profile root). Aligns the mock with `cmaxctl.paths.caam_profile_creds_path`.
